...
export default function Web3Provider({ children }: Web3ProviderProps) {
  ...
  const [rainbowKitConfig] = useState(() => {
    ...
    if (
      isDevelopment &&
      (!projectId || projectId === "valentine-memory-game")
    ) {
      ...
      if (!projectId || projectId === "valentine-memory-game") {
        console.warn(
          "Using fallback WalletConnect project ID. Add NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID to enable full features."
        );
      }
    }
  ...
  const [isMiniApp] = useState(() => {
    ...
    const hasFarcasterContext = !!(window as any).fc;

    // Check user agent for Farcaster app
    const userAgent = navigator.userAgent.toLowerCase();
    const isFarcasterApp = userAgent.includes("farcaster");

    const result = hasUrlIndicators || hasFarcasterContext || isFarcasterApp;
