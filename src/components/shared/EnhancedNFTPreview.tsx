...
interface EnhancedNFTPreviewProps {
  images: string[];
  message: string;
  users?: FarcasterUser[];
  gameStats?: {
...
export default function EnhancedNFTPreview({
  images,
  message,
  users = [],
  gameStats,
  ...
  onUserClick,
}: EnhancedNFTPreviewProps) {
  const [activeView, setActiveView] = useState<'heart' | 'social' | 'stats'>('heart');
  const [hoveredUser, setHoveredUser] = useState<number | null>(null);

  const displayImages = images.slice(0, 8);
  const featuredUsers = users.slice(0, 8);
  const powerBadgeUsers = users.filter(u => u.power_badge);
          ...
          >
              ...
              <div className="grid grid-cols-4 gap-3">
                {featuredUsers.map((user, index) => (
                  ...
                  >
                    ...
                    {user.power_badge && (
                      <div className="absolute -top-1 -right-1 w-6 h-6 bg-yellow-400 rounded-full flex items-center justify-center text-xs shadow-lg">
                        ⚡
                      </div>
                    )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl p-4 text-white">
                <div className="text-2xl font-bold">{powerBadgeUsers.length}</div>
                <div className="text-sm opacity-90">Power Badge Holders</div>
              </div>
      ...
      {tokenId && (
        <div className="mt-6 text-center">
          <div className="inline-flex items-center gap-2 bg-white rounded-full px-4 py-2 shadow-lg border border-gray-200">
            ...
            <span className="text-sm font-bold text-purple-600">#{tokenId}</span>
          </div>
        </div>
      )}
    </div>
